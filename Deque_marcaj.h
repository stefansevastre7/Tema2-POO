#ifndef DEQUE_MARCAJ_H_INCLUDED
#define DEQUE_MARCAJ_H_INCLUDED
#include "Coada.h"
...
public:
    Deque_Marcaj(Nod *prim,Nod *ultim, int dim_max)
            : Coada(prim,ultim,dim_max)
    {

    }
    Deque_Marcaj(const Deque_Marcaj &ob)
            : Coada(ob)
    {
    ...
    void pop()
    {
        try {
            std::string t;
            std::cout << "pop() called for deque marcaj. Type 'front' or 'back': ";
            cin >> t;
            if(t!="front" && t!="back")
                throw "eroare";
            else
    ...
    friend istream& operator>> (istream &in, Deque_Marcaj &a)
    {
        return in;
    }
    friend ostream& operator<< (ostream &out, Deque_Marcaj &a)
    {
    ...
    Deque_Marcaj& operator=(Deque_Marcaj &a) {
        empty();
